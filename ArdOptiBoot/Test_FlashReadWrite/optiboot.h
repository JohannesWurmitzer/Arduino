...
#else
#if FLASHEND > 65534
const do_spm_t do_spm = (do_spm_t)((FLASHEND - 4095 + 2) >> 1);
#else
...
void optiboot_writePage(uint32_t addr, uint8_t data_to_store[], uint16_t page)
{
  uint16_t word_buffer = 0;

  // Erase the flash page
  optiboot_page_erase((optiboot_addr_t)(addr + (SPM_PAGESIZE * (page - 1))));
  ...
  for (uint16_t i = 0; i < SPM_PAGESIZE; i++)
  {
    ...
    else
    {
      ...
      optiboot_page_fill((optiboot_addr_t)(addr + i + (SPM_PAGESIZE * (page - 1))), word_buffer);
    }
  }

  // Writing temporary buffer to flash
  optiboot_page_write((optiboot_addr_t)(addr + (SPM_PAGESIZE * (page - 1))));
}
 ...
 * @brief 
 * 
 * @param allocated_flash_space 
 * @param storage_array 
 * @param page 
 * @param blank_character 
 */
void optiboot_readPage(uint32_t addr, uint8_t storage_array[], uint16_t page, char blank_character)
{
  uint8_t read_character;
  for (uint16_t j = 0; j < SPM_PAGESIZE; j++)
  {
    read_character = pgm_read_byte(addr + j + (SPM_PAGESIZE * (page - 1)));
...
void optiboot_readPage(uint32_t addr, uint8_t storage_array[], uint16_t page)
{
  uint8_t read_character;
  for (uint16_t j = 0; j < SPM_PAGESIZE; j++)
  {
    read_character = pgm_read_byte_far(addr + (j + SPM_PAGESIZE * (page - 1)));
    storage_array[j] = read_character;
  }
}
